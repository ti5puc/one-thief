...
public class SpikeTrapPart : MonoBehaviour
{
    ...
    public void Activate(float totalDuration)
    {
        ...
        seq.Append(transform.DOLocalMoveY(initialPositionY + preDelayMoveY, safePreDelay));
        if (safeInterval > 0f)
            seq.AppendInterval(safeInterval);
        seq.Append(transform.DOLocalMoveY(initialPositionY + spikeMoveY, safeSpikeMove));
    }
